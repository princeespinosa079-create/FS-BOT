...
// ============================================================
// ✅ SLASH COMMANDS — ALL HAVE PROPER DESCRIPTIONS
// ============================================================
const commands = [
  new SlashCommandBuilder()
    ...
    .addStringOption(o => o.setName("channel_id").setDescription("Raw channel ID.")),
  new SlashCommandBuilder()
    .setName("say")
    ...
    .addStringOption(o => o.setName("type").setDescription("Message style.").setRequired(true).addChoices({ name: "With Embed", value: "good" }, { name: "No Embed", value: "none" }))
  ...
  new SlashCommandBuilder()
    ...
    .addStringOption(o => o.setName("source_id").setDescription("Raw source channel ID."))
    .addChannelOption(o => o.setName("destination").setDescription("Destination channel."))
    .addStringOption(o => o.setName("destination_id").setDescription("Raw destination channel ID.")),
  new SlashCommandBuilder()
    .setName("setchannel")
...
client.once("ready", () => {
  ...
  console.log(`📚 Files loaded: ${library.files.length}`);
  console.log("==========================================");
  registerCommands();
});
...
client.on("interactionCreate", async interaction => {
  ...
  try {
    ...
    if (interaction.commandName === "say") {
      ...
      else { const emb = new EmbedBuilder().setColor(0x808080).setDescription(text).setFooter({ text: time }); if (title) emb.setTitle(title); await interaction.channel.send({ embeds: [emb] }); }
      return;
    }
...
// ============================================================
// ✅ DOT COMMANDS — DM + SERVER, OWNER + ACCESS ROLE
// ============================================================
client.on("messageCreate", async msg => {
  ...
  if (/^\.(removeline|rl)$/i.test(txt)) {
    ...
    setTimeout(async () => { try { const res = await fetch(file.url); const t = await res.text(); const clean = t.replace(/--.*$/gm, "").split("\n").filter(l => l.trim() !== "").join("\n"); const out = new AttachmentBuilder(Buffer.from(clean), { name: "prince_is_the_best.lua" }); if (sent) await sent.delete().catch(() => {}); await msg.channel.send({ content: `<@${msg.author.id}> **Here is the file bro!**`, files: [out] }).catch(() => {}); } catch (e) { if (sent) await sent.delete().catch(() => {}); replyUser(msg, `❌ error: ${e.message}`).catch(() => {}); } }, isOwnerUser ? 0 : 3000);
    return;
  }
  if (/^\.get(?:\s|$)/i.test(txt)) {
...
client.login(TOKEN).catch(e => { console.error("❌ Login fail:", e.message); process.exit(1); });
